const AutoRemoveDelta = 3600 * 1000; // How long we can remove an accessory when there's no update.
...
AqaraAccessoryFactory.prototype.configureAccessory = function(accessory) {
  ...
  this.lastUpdateTime[accessory.UUID] = Date.now();
}
...
AqaraAccessoryFactory.prototype.autoRemoveAccessory = function() {
  ...
  for (var i = this.accessories.length - 1; i--; ) {
    ...
    var lastTime = this.lastUpdateTime[accessory.UUID];
    var delta = now - lastTime;
    // this.log.debug("%s delta %d", accessory.UUID, delta);
    if (delta > AutoRemoveDelta) {
...
AqaraAccessoryFactory.prototype.setTemperatureAndHumidity = function(deviceSid, temperature, humidity) {
...
// Motion sensor
AqaraAccessoryFactory.prototype.setMotion = function(deviceSid, motionDetected) {
  // use last four characters of deviceSid as service name
  var deviceName = deviceSid.substring(deviceSid.length - 4);
...
// Contact sensor
AqaraAccessoryFactory.prototype.setContact = function(deviceSid, contacted) {
  // use last four characters of deviceSid as service name
  var deviceName = deviceSid.substring(deviceSid.length - 4);
...
// Light switch
AqaraAccessoryFactory.prototype.setLightSwitch = function(deviceSid, uuidSeed, on, commander) {
  // use last four characters of deviceSid as service name
  var deviceName = deviceSid.substring(deviceSid.length - 4);
...
AqaraAccessoryFactory.prototype.findServiceAndSetValue = function(
  ...
  this.lastUpdateTime[newAccessory.UUID] = Date.now();
